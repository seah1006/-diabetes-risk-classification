...
03 통계 분석   카이제곱 / t-검정 / 로지스틱 회귀 오즈비 → 위험 요인 규명
    ↓
04 의사결정    위험 단계 분류 → 고위험군 프로파일 → 예방 정책 시나리오 4개
```
...
- **피처 제거하지 않음**: BRFSS 피처는 의학적으로 독립적 위험 요인으로 정의된 항목이므로 통계적 공선성이 있어도 임상적 의미를 보존하기 위해 전체 유지한다. 로지스틱 회귀의 기본 L2 정규화가 공선성 영향을 일부 완화한다. 단, Education·CholCheck의 오즈비는 공선성으로 인해 단독 효과로 해석하기 어렵다는 점을 유의한다.
...
- 로지스틱 회귀는 **전처리 완료 데이터**(`train_test_split.pkl`)를 사용한다 — SMOTE로 클래스 비율을 50:50으로 균형 보정한 후 학습하므로 절대적 위험 확률이 아닌 **피처 간 상대적 영향력 비교** 목적으로 오즈비를 해석한다. 스케일링 완료 데이터를 쓰는 이유는 피처 단위가 다르면 계수 크기를 직접 비교할 수 없기 때문이다.
...
- ※ GenHlth(1~5 리커트 척도)는 엄밀히 순서형이나 연속형으로 취급해 t-검정을 적용했다. 비모수 대안(Mann-Whitney U)과 결과가 유사하다고 가정하며, BRFSS 데이터의 관행적 처리 방식을 따른 것이다.
...
임계값 근거: 섹션 5의 Precision-Recall 트레이드오프 분석에서 F1 최대 임계값과 Recall 0.8 보장 임계값(약 0.46)을 확인한 후, 미탐지 비용이 오탐지 비용보다 크다는 프로젝트 기준에 따라 낮은 쪽인 0.3을 중간 구간 하한으로 설정했다. 이 기준은 절대적인 의학 기준이 아닌 모델 확률을 정책적으로 해석하기 위한 분석 기준임을 명시한다.
